...
fn get_def_from_doc(doc: &Document) -> Option<DictDef> {
    if let Some(panel) = doc.find(Class("def-panel")).into_selection().first() {
        if let Some(word) = panel.find(Class("word")).into_selection().first() {
            ...
            let sounds = panel
                .find(Class("play-sound"))
                .into_selection()
                .first()
                .and_then(|e| e.attr("data-urls").and_then(|s| Some(s.to_owned())))
                .and_then(|l| Some(json_list_to_strings(&l)));

            let similars = Some(
                doc.find(Name("ul").and(Class("alphabetical")))
                    .into_selection()
                    .find(Name("li"))
                    .find(Name("a"))
                    .iter()
