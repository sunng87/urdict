            ...
            match pieces[0].to_lowercase().as_ref() {
                "define" => {
                    ...
                    if let Some(def) = page::find_on_urban_dict(&word) {
                        stream
                            .write(format!("150 {} definitions retrieved\r\n", 1).as_bytes())
                            .unwrap();

                        stream
                            .write(format!("151 \"{}\" urdict", def.word).as_bytes())
                            .unwrap();
                        stream
                            .write(
                                format!(
                                    " \"Urban Dictionary {} {}\"\r\n",
                                    def.contributor, def.date
                                ).as_bytes(),
                            )
                            .unwrap();
                        stream
                        ...
                        stream
                            .write(format!("150 0 definitions retrieved\r\n").as_bytes())
                            .unwrap();
                    }
                    stream.write("250 ok\r\n".as_bytes()).unwrap();
                }
                "match" => {
                    ...
                    if let Some(def) = page::find_on_urban_dict(&word) {
                        if let Some(similars) = def.similars {
                            stream
                                .write(
                                    format!("152 {} matches found\r\n", similars.len()).as_bytes(),
                                )
                                .unwrap();
                            for s in similars {
                                stream
                ...
                "show" => match pieces[1].to_lowercase().as_ref() {
                    "db" => {
                        stream
                            .write("110 1 database present\r\n".as_bytes())
                            .unwrap();
                        stream
                            .write("urdict \"Urban Dictionary\"\r\n".as_bytes())
                            .unwrap();
                        stream.write(".\r\n".as_bytes()).unwrap();
                        stream.write("250 ok\r\n".as_bytes()).unwrap();
                    }
